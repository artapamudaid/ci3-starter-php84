...
class RolePermission extends MY_Controller {
    ...
    public function index(){
        ...
        $this->render('role_permission/index', $data);
    }
    ...
    public function assign($role_id) {
        ...
        if ($this->input->method() == 'post') {
            ...
            foreach($data['menus'] as $m){
                ...
                $this->RolePermission_model->upsert($role_id, $menuId, [
            ...
            redirect('RolePermission/assign/'.$role_id);
        }

        $existing = [];
        foreach($this->RolePermission_model->get_by_role($role_id) as $p){
            $existing[$p->menu_id] = $p;
        }
        $data['existing'] = $existing;
        $this->render('role_permission/assign', $data);
    }
